...
also picks the 32-bit x86 cross toolchain (default `CC32`/`AR32`) since qc--
only ever emits i386, and a `RUN32` emulator when the host isn't x86 — by
asking `qc` itself (`qc -x86 -print-cc`/`-print-ar`/`-print-run`) rather than
detecting a toolchain on its own; this repo does no toolchain detection of
its own for any backend, qc-- centralizes all of it (see qc--'s own
`docs/claude_notes/plan_toolchain_dispatcher.txt`). Options: `--qc=PATH`,
`--cc32=CMD`, or set `QC`/`CC32`/`RUN32`/`AR32` in the environment — any of
these you set explicitly is used as-is instead of asked of `qc`.
`Makefile.config` is generated by this script — never edit it, re-run
`./configure` instead. `configure.old` is the original Perl script, kept for
...
qc-- targets more than x86 (see `qc --help`): `-ppc`, `-sparc`, `-alpha`,
...
asks `qc` for each backend's optional cross toolchain and, where relevant, its
`qemu-user` emulator (`CC_<ARCH>`/`AR_<ARCH>`/`RUN_<ARCH>` in
`Makefile.config`, best-effort — a missing one only warns) rather than
detecting either itself.
