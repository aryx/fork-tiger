...
clean::
	rm -rf tests/build $(addprefix tests/build-,$(ARCHS))
install::
	dune install
uninstall::
	dune uninstall
...
# one (see "qc --help" for the full list, or Makefile.backends' ARCHS).
# Kept as their own targets rather than folded into "test" - each needs
# its own cross toolchain (and usually a qemu-user emulator, see
# ./configure's output), and none is expected to be all-green: see
# tests/expected/tiger-<arch>.txt and tests/run-tests.sh's header for the
# known qc-- backend gaps each has already turned up.
#
# claude: this used to be 12 copy-pasted 3-line targets (one per
# Makefile.backends' ARCHS entry, each identical but for the backend
# name) - synthesized instead via $(eval $(call ...)), the standard GNU
# Make idiom for "the same few rules, once per item in a list" (see the
# GNU Make manual's "Eval Function" chapter). arm64/amd64 need no cross
# toolchain at all on their own native host (this dev box's aarch64-linux
# included) - see ./configure's own arm64 comment; arm64-mach-o/
# amd64-mach-o are the OLD macOS-only Mach-O backends those two flags
# used to mean, kept out of test-all below same as before, only ever
# buildable on Darwin.
define TEST_ARCH_template
test-$(1)::
	$$(MAKE) -C stdlib BACKEND=$(1)
	$$(MAKE) -C runtime BACKEND=$(1)
	BACKEND=$(1) tests/run-tests.sh
endef
$(foreach a,$(ARCHS),$(eval $(call TEST_ARCH_template,$(a))))
...
# claude: DARWIN_ARCHS (arm64-mach-o, amd64-mach-o) deliberately excluded
# here, same reasoning as every other backend whose cross toolchain isn't
# assumed to be installed.
test-all:: test $(addprefix test-,$(filter-out $(DARWIN_ARCHS),$(ARCHS)))
